...
from .attention_utils import PositionalEmbedding, PatchMerging_v2
from .common_function import get_conv, get_norm, get_act
from .EMCAD3 import MSUNeXtBlock
...
class MultiModal_Paired_Windows_Attention(Paired_Windows_Attention):
    ...
    def __init__(self,
        ...
        if self.num_heads > 0:
            ...
            self.mix_channels = get_conv(dim)(mid_channels*4, in_channels, kernel_size=1, groups=num_modalities)
...
class Paired_Windows_TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        groups = sum_channels // groups_size
        
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm = get_norm(conv_norm_type, dim)(sum_channels)
        self.ffn = MSUNeXtBlock(sum_channels, kernel_size, ffn_expansion_ratio, groups, act_layer, proj_drop, dim)

    def forward(self, xs: torch.Tensor):
        attns = self.norm(xs + self.drop_path(self.attn(xs)))
        attns = attns + self.ffn(attns)
        return attns
...
class Transformer_BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        self.downs = None
        if do_downsample:
            self.downs = PatchMerging_v2(in_ch=sum(in_channels), dim=dim)
