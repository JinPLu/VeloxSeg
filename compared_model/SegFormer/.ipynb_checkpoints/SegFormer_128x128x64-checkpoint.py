...
class SegFormer3D(nn.Module):
    def __init__(
        ...
        reversed_embed_dims = embed_dims[::-1] # reversed_embed_dim
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, H, W, D):
        ...
        k, v = kv[0], kv[1]
#         print('----------------------------')
#         print(k.shape, v.shape, q.shape, self.sr_ratio)
...
#         print(f'attnention_prob, {attnention_prob.shape}')
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, H, W, D):
        ...
        x = x + self.mlp(self.norm2(x), H, W, D)
        return x
...
class MixVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # (num_patches,) -> (H, W, D)
        # (batch_size, num_patches, hidden_state) -> (batch_size, hidden_state, H, W, D)
        ...
        B, N, C = x.shape
        # n = cube_root(N)
        H, W, D = H//self.patch_stride[0], W//self.patch_stride[0], D//self.patch_stride[0]
        for i, blk in enumerate(self.tf_block1):
            x = blk(x, H, W, D)
        x = self.norm1(x)
        # (B, N, C) -> (B, H, W, D, C) -> (B, C, H, W, D)
        ...
        x = self.embed_2(x)
        B, N, C = x.shape
        H, W, D = H//self.patch_stride[1], W//self.patch_stride[1], D//self.patch_stride[1]
        ...
        x = self.embed_3(x)
        B, N, C = x.shape
        H, W, D = H//self.patch_stride[2], W//self.patch_stride[2], D//self.patch_stride[2]
        ...
        x = self.embed_4(x)
        B, N, C = x.shape
        H, W, D = H//self.patch_stride[3], W//self.patch_stride[3], D//self.patch_stride[3]
