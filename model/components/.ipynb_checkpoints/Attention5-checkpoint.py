...
from .EMCAD2 import MSUNeXtBlock
...
class Paired_Windows_TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm = get_norm(conv_norm_type, dim)(sum(in_channels))
        
        sum_channels = sum(in_channels)
        mid_channels = sum_channels // self.num_modalities
        self.ffn = nn.Sequential(
            get_conv(dim)(sum_channels, mid_channels, 3, 1, 1),
            get_norm(conv_norm_type, dim)(mid_channels),
            get_act(act_layer, inplace=True),
            get_conv(dim)(mid_channels, sum_channels, 3, 1, 1),
