...
from compared_model.A2FSeg.nnunet.network_architecture.multi_modal.utils import subset_idx

from copy import deepcopy
from compared_model.A2FSeg.nnunet.utilities.nd_softmax import softmax_helper
from torch import nn
import torch
import numpy as np
from compared_model.A2FSeg.nnunet.network_architecture.initialization import InitWeights_He
from compared_model.A2FSeg.nnunet.network_architecture.my.neural_network3 import SegmentationNetwork
...
class Generic_UNet(nn.Module):
    ...
    DEFAULT_PATCH_SIZE_3D = (64, 64, 32)
    ...
    # input_channels=2  base_num_features=32 num_classes=2 num_pool=5
...
##############################################################################
        drop = np.array([True if k in SUBSETS_MODALITIES[subset_idx_list[0]] else False for k in range(self.modality_num)]) == False
##############################################################################
        # print(drop)
...
##############################################################################
        ...
        final_outputs = [torch.nn.functional.interpolate(pred, size=[h, w, d], mode='trilinear', align_corners=True) for pred in final_outputs]
        return final_outputs
