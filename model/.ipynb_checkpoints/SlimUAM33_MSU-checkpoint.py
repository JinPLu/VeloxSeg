...
class SlimUM_Decoder(nn.Module):
    def __init__(self, patch_size: int, base_ch: int = 32, out_ch: int = 2, 
        ...
        self.layer_up3 = nn.Sequential(transpose_conv(base_ch*8, base_ch*4, kernel_size=2, stride=2), 
                                    norm(base_ch*4),
                                    nn.GELU())
        self.layer_up2 = nn.Sequential(transpose_conv(base_ch*4, base_ch*2, kernel_size=2, stride=2), 
                                    norm(base_ch*2),
                                    nn.GELU())
        self.layer_up1 = nn.Sequential(transpose_conv(base_ch*2, base_ch, kernel_size=2, stride=2), 
                                    norm(base_ch),
                                    nn.GELU())
