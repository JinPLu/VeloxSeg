...
class ResidualDoubleConv(nn.Module):
    def __init__(self, in_ch, out_ch, groups_size=None, reduction=1, dropout=0.0, dim = 3):
        super().__init__()
        hidden_ch = out_ch // reduction
        groups = 1 if groups_size is None else in_ch // groups_size
        if dim == 3:
            conv = nn.Conv3d
            norm = nn.InstanceNorm3d
        else:
        ...
        self.conv = nn.Sequential(
            conv(in_ch, hidden_ch, kernel_size=3, stride=1, padding=1, groups=groups),
            norm(hidden_ch),
            nn.GELU(),
            conv(hidden_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=groups),
            norm(out_ch),
            nn.Dropout(dropout),
        )
        self.residual = nn.Sequential(
            conv(in_ch, out_ch, 1, 1, groups=groups),
            norm(out_ch),
            nn.Dropout(dropout),
        )
        self.relu = nn.GELU()
...
class DownConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, patch_size=2, groups=1, dim=3):
