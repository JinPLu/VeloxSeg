...
from nnunetv2.training.nnUNetTrainer.variants.network_architecture.model.components.common_function import get_conv, get_norm, get_pool, get_act
from nnunetv2.training.nnUNetTrainer.variants.network_architecture.model.components.unet_blocks import UpsampleConv


class MSUNeXtBlock(nn.Module):

    def __init__(self, in_channels, kernel_sizes=((1, 1, 1), (3, 3, 3), (5, 5, 5)), groups=1, epansion_factor=4,
        ...
        if len(kernel_sizes) > 1:
            self.spatial_convs = nn.ModuleList([
                nn.Sequential(
                    conv(in_channels, in_channels, kernel_size, padding=tuple(k // 2 for k in kernel_size), groups=groups),
                    norm(in_channels),
            ...
            self.spatial_convs = nn.ModuleList([
                conv(in_channels, in_channels, kernel_sizes[0], padding=tuple(k // 2 for k in kernel_sizes[0]), groups=groups)
            ])

        # self.channel_conv = nn.Sequential(
        #     conv(in_channels, in_channels, 1, 1, 0),
        #     norm(in_channels),
        #     nn.Dropout(dropout)
        # )
        self.channel_conv = nn.Sequential(
...
def MSUNeXtLayer(in_channels, depth=1, kernel_sizes=((1, 1, 1), (3, 3, 3), (5, 5, 5)), groups=1, epansion_factor=4,
                 activation='gelu', dropout=0.0, spatial_dim=3):

        msconvs = nn.Sequential(*[
                        MSUNeXtBlock(in_channels, kernel_sizes=kernel_sizes, groups=groups, epansion_factor=epansion_factor,
...
#     def __init__(self, channels, k_s, k_l, groups=1, activation="gelu", spatial_dim=3):
#         super(CAG, self).__init__()

#         self.conv_small = get_conv(spatial_dim)(channels, channels, k_s, 1, k_s // 2, groups=groups)
#         self.conv_large = get_conv(spatial_dim)(channels, channels, k_l, 1, k_l // 2, groups=groups)

#         self.norm = get_norm("IN", spatial_dim)(channels * 2)
#         self.act_layer = get_act(activation)

#         self.channel_fusion = nn.Sequential(
#             get_conv(spatial_dim)(channels * 2, channels, 1, 1, 0),
#         )
#     def forward(self, f_s, f_l):
#         fea = torch.cat([self.conv_large(f_s), self.conv_small(f_l)], dim=1)
#         fea = self.act_layer(self.norm(fea))
#         fea = self.channel_fusion(fea)
...
class CAG(nn.Module):
    ...
    def __init__(self, channels, k_s, k_l, groups=1, activation="gelu", spatial_dim=3):
        ...
        self.conv_small = nn.Sequential(
            get_conv(spatial_dim)(channels, channels, k_s, 1, k_s // 2, groups=groups),
            get_norm("IN", spatial_dim)(channels),
            get_act(activation, inplace=True)
        )
        self.conv_large = nn.Sequential(
            get_conv(spatial_dim)(channels, channels, k_l, 1, k_l // 2, groups=groups),
            get_norm("IN", spatial_dim)(channels),
            get_act(activation, inplace=True)
        )
    ...
    def forward(self, f_s, f_l):
        fea = torch.cat([self.conv_large(f_s), self.conv_small(f_l)], dim=1)
        mask = self.channel_fusion(fea)

        return f_s * (1 + mask) + f_l
