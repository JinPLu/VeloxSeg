...
from nnunetv2.training.nnUNetTrainer.variants.network_architecture.model.components.attention_utils import FFN, LayerNorm, PositionalEmbedding, PatchMerging
...
class Transformer_BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        self.downs = nn.ModuleList([
            PatchMerging(in_ch=cin, out_ch=cout, stride=down_stride, norm_layer=norm_layer, dim=dim)
            for cin, cout in zip(in_channels, down_channels)])
