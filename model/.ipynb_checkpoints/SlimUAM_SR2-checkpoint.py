...
class SlimUAM_SR(SlimUAM):
    ...
    def __init__(self,
        ...
        self.sr_decoders = nn.ModuleList([SR_Decoder(in_ch[m], patch_size, base_ch // self.num_modalities, spatial_dim=spatial_dim) for m in range(self.num_modalities)])
    ...
    def forward(self, x) -> Union[torch.Tensor, Sequence[torch.Tensor]]:
        ...
        if self.training:
            sr = [self.sr_decoders[m](attn1[m], attn2[m], attn3[m], attn4[m]) for m in range(self.num_modalities)]
